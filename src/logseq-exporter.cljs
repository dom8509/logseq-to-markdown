...
(defn- parse-meta-data
  ...
  (let [original-name (get page :block/original-name)
        ...
        file (str (->filename (or (and namespace? (last (s/split original-name "/"))) original-name)) ".md")
      ...
      (if (empty? link-res)
        ...
        (reduce
         #(let [current-text (first %2)
                ...
                replaced-str (or (and (page-exists? current-link) (str "[[[" link-text "]]]({{< ref \"/pages/" (->filename current-link) "\" >}})"))
                                 (str link-text))]
    ...
    (if (empty? res)
      ...
      (let [namespace-name (last res)
            query '[:find ?n
                    :in $ ?namespace-name
                    :where
                    [?p :block/original-name ?namespace-name]
                    [?c :block/namespace ?p]
                    [?c :block/original-name ?n]
                    [?c :block/properties ?pr]
                    [(get ?pr :public) ?t]
                    [(= true ?t)]]
            query-res (d/q query (get-graph-db) namespace-name)]
        (if (empty? query-res)
          (str text)
          (let [data (map #(get % 0) query-res)
                heading (str "***Namespace " 
                             (if (page-exists? namespace-name)
                               ...
                               (str namespace-name "***\n")))
                prefix (if (get-exporter-config :keep-bullets)
                         (str (apply str (concat (repeat (* level 1) "\t"))) "+ ")
                         (str (apply str (concat (repeat (* (- level 1) 1) "\t"))) "+ "))
                content (reduce #(str %1 prefix "[" %2 "]({{< ref \"/pages/" (->filename %2) "\" >}})\n") "" data)]
            (str heading content)))))))
